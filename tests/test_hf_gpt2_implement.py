...
def test_mistral_gpt2_roundtrip():
    _roundtrip_compare_gpt2_checkpoint("/nlp/scr/kathli/checkpoints/devout-shadow-123/step-20000/hf", "/nlp/scr/kathli/checkpoints/devout-shadow-123/step-20000", None)
...
def get_gpt2_model(checkpoint_path):
    ...
    trainer.parameter_axis_resources = {"embed": "data"}
    ...
    def init_model():
        model = Gpt2LMHeadModel(Vocab, config_model, key=model_key)
        return mp.cast_to_param(model)

    model = init_model()
    optimizer = trainer.optimizer()
    opt_state = model #named_pjit(optimizer.init, axis_resources=parameter_axis_mapping)

    checkpointer = trainer.checkpointer.create("devout-shadow-123")

    model, (opt_state, training_key), resume_step = checkpointer.load_checkpoint(
        model,
        (opt_state, training_key),
        "/nlp/scr/kathli/checkpoints/devout-shadow-123"
    )
    '''
    vocab_size = 55028 #len(tokenizer)
    Vocab = Axis("vocab", vocab_size)
