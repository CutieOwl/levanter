...
def test_accumulate_gradients_sharded():
    In = hax.Axis("In", 32)
    Out = hax.Axis("Out", 32)
    Mid = hax.Axis("Mid", 32)
    ...
    @hax.partitioning.named_pjit(axis_resources=axis_mapping)
    def jit_grad_accum(mlp, x):
        acc_v, acc_g = accumulate_gradients_sharded(grad_fn, Batch, mlp, x, per_device_parallelism=1, parameter_axis_mapping=axis_mapping)
        return acc_v, acc_g
