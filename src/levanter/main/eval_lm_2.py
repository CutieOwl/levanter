...
def main(config: EvalLmConfig):
    ...
    print(eval_idx)
    config.data.train_urls = ["gs://pubmed-mosaic/openwebtext-sharded/openwebtext_train.{1..128}-of-128.jsonl.gz"]
    config.data.validation_urls = ["gs://pubmed-mosaic/openwebtext-sharded/openwebtext_val.{1..8}-of-8.jsonl.gz"]
    config.data.cache_dir = "gs://levanter-data/tokenized/openwebtext/"
    config.data.tokenizer = "gpt2"
    print(config.data)
    tokenizer = config.data.the_tokenizer

    Batch = Axis("batch", config.trainer.eval_batch_size)
    Pos = config.model.Pos(eval_idx)
    KeyPos = config.model.KeyPos(eval_idx)
