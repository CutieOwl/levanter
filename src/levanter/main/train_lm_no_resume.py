...
@dataclass
class TrainLmConfig:
    data: List[LMDatasetConfig] = field(default_factory=list)
...
def main(config: TrainLmConfig):
    ...
    # some axes we need
    Batch_axes = []
    for batch_size in config.trainer.train_batch_size:
        Batch_axes.append(Axis("batch", batch_size))
    EvalBatch = Axis("batch", config.trainer.eval_batch_size)
    Pos_axes = []
    KeyPos_axes = []
    for i in range(len(config.model.seq_len)):
        Pos_axes.append(config.model.Pos(i))
        KeyPos_axes.append(config.model.KeyPos(i))
    ...
    for i in range(len(config.data)):
        train_loaders.append(ShardedBatchLoader(
            CausalLmDataset(config.data[i].token_seq_dataset("train", Pos_axes[i].size), Pos_axes[i], KeyPos_axes[i]),
            # TokenSeqDataset(config.data.build_or_load_cache("train"), Pos),
            config.trainer.device_mesh(i),
            Batch_axes[i],
            compute_axis_mapping,
        ))
    ...
    with config.trainer.device_mesh(0) as mesh:
        ...
        model_discard, opt_state = named_jit(init_model_and_opt_state, axis_resources=parameter_axis_mapping)(
            model_key
        )
