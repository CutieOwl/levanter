...
class TrainerConfig:
    ...
    model_axis_size: List[int] = field(default_factory=lambda: [1])  # how many devices to shard each model over. Data axis is the other axis

    # Config related to batch sizes
    train_batch_size: List[int] = field(default_factory=lambda: [512])
    step_curriculum: List[int] = field(default_factory=lambda: [1])
    eval_idx: int = 0
    per_device_parallelism: List[int] = field(default_factory=lambda: [-1])
